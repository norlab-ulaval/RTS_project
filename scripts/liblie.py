...
def expm(M):
    ...
    if M.shape[0] != M.shape[1]:
        raise ValueError('expm: M should be a square skew-symmetric matrix ' + str(M.shape))
...
def expm_so3(M):
    ...
    """
    if M.shape != (3,3):
        raise ValueError('expm_so3: M should be a 3x3 skew-symetric matrix and not ' + str(M.shape))
...
def jac_so3(e):
    ...
    return np.sin(phi)/phi*np.eye(3,3) + (1.-np.sin(phi)/phi)*xxT(a) + (1.-np.cos(phi))/phi*wedge_so3(a)
...
def jac_inv_so3(e):
    ...
    a = e/scipy.linalg.norm(e)
    phi=scipy.linalg.norm(e)
    return phi/2.*cot(phi/2.)*np.eye(3,3) + (1-phi/2.*cot(phi/2.))*xxT(a) - phi/2. * wedge_so3(a)
...
def logm_se3(T):
    ...
    """
    if T.shape != (4,4):
        raise ValueError('logm_se3: T should be a 4x4 transformation matrix and not ' + str(T.shape))
